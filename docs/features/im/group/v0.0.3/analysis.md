...
v0.0.1 实现了群聊创建，v0.0.2 补全了搜索加群与入群审批，并提供了最小的群详情页（成员列表 + 入群验证开关）。本次要给群聊装上完整的"治理能力"：邀请新成员、踢人、退群、转让群主、解散群聊、群公告、群设置（修改群名）。
...
群主在群详情页点击"-"按钮，进入移除成员选人页（MemberPickerPage 移除模式，红色勾选）。选择要移除的成员后点击"移除"按钮，逐个移除选中成员，刷新宫格头像，发系统消息"XXX 被移出群聊"。被踢的成员的会话列表中该群聊标记为已删除。

```mermaid
flowchart LR
    A[群详情页] --> B[群主点击 - 移除]
    B --> C[MemberPickerPage 移除模式]
    C --> D[选择成员 + 点击移除]
    D --> E[逐个移除 + 系统消息]
```
...
群主在群详情页底部点击"转让群主"，弹出成员选择列表（排除自己）。选择新群主后确认，`conversations.owner_id` 更新，发系统消息"XXX 将群主转让给了 YYY"。转让后原群主变为普通成员，群详情页的权限按钮随之变化。
...
群主在群详情页底部点击"解散群聊"，弹出确认对话框（二次确认，因为不可逆）。确认后：发系统消息"群聊已解散"，conversations.status 标记为 1（已解散）。群聊不删除成员和消息，历史消息仍可查看但不能再发送。群主自动返回会话列表。
...
群主在群详情页点击"群公告"入口，进入群公告页面。如果没有公告显示"暂无群公告"+ 发布按钮；如果有公告显示公告内容。群主可以编辑/发布公告（TextField + 发布按钮），普通成员只能查看。发布/编辑公告后发系统消息"XXX 更新了群公告"，并通过 GROUP_INFO_UPDATE WS 帧实时推送给所有群成员。
...
群主在群详情页点击"群聊名称"行，跳转独立的编辑群名页面（EditGroupNamePage）。修改群名后发系统消息"XXX 将群名修改为「YYY」"，并通过 GROUP_INFO_UPDATE WS 帧实时同步到所有成员的会话列表和 ChatPage 标题。

```mermaid
flowchart LR
    A[群详情页] --> B[点击群聊名称]
    B --> C[EditGroupNamePage]
    C --> D[修改群名 + 系统消息]
    D --> E[GROUP_INFO_UPDATE 推送]
```
...
| T4 | 系统消息 + 推送 | send_system "XXX 更新了群公告" + GROUP_INFO_UPDATE 推送给所有群成员 | 所有成员收到 |
...
| T4 | 系统消息 + 推送 | send_system "XXX 将群名修改为「YYY」" + GROUP_INFO_UPDATE 推送给所有群成员 | 所有成员收到 |
...
| D-30 | 修改群名 | 领域 | PUT /groups/{id}，群主修改群名 |
| P-38 | 群详情页扩展 | 前端业务 | 邀请/踢人/退群入口 + 群公告 + 群名编辑 + 转让/解散独立按钮 |
| P-39 | 邀请入群选人页 | 前端业务 | 复用 CreateGroupPage 选人交互，已有成员预选不可取消 |
| P-40 | 群公告页 | 前端业务 | 查看/编辑群公告，群主可发布 |
| F-11 | GROUP_INFO_UPDATE WS 帧分发 | 基础设施 | 群信息变更（改名/改公告/解散）实时推送给所有群成员 |
| I-10 | AppError 统一错误处理 | 基础设施 | flash-core 统一错误类型，替代 map_err 反模式 |
...
| F-06 | WsClient 新增 groupInfoUpdateStream（监听 GROUP_INFO_UPDATE 帧） |
...
| GROUP_INFO_UPDATE proto 帧 | F-11 | F-06 / P-38 | 新增 WS 帧类型，群信息变更实时推送 |

---

## 四、结论

- **开发顺序**：数据库迁移（conversations 加 status 字段 + group_info 加 announcement 相关字段）→ D-24 邀请入群 → D-25 踢人 → D-26 退群 → D-27 转让 → D-28 解散 → D-29 群公告 → D-30 修改群名 → MessageService.send 拦截已解散群 → P-38 群详情页扩展 → P-39 邀请选人页 → P-40 群公告页 → ChatPage 已解散状态适配 + 群公告横幅 + GROUP_INFO_UPDATE 实时同步
- **复杂度集中点**：
  ...
  - GROUP_INFO_UPDATE 实时推送：改名/改公告时通过独立 WS 帧推送给所有群成员，驱动会话列表和 ChatPage 实时更新
