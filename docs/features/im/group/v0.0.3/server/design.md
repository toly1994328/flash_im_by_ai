...
- 新增 `PUT /groups/{id}` 接口：群主修改群名
- 新增 GROUP_INFO_UPDATE WS 帧类型：改名/改公告时推送给所有群成员
- 新增 AppError 统一错误处理：替代 `map_err(|_| StatusCode)` 反模式
...
- 无 GROUP_INFO_UPDATE WS 帧类型
- 无统一错误处理（handler 直接返回 StatusCode）
...
sequenceDiagram
    ...
    C->>API: PUT /groups/{id}
    API->>DB: 校验群主身份 + 更新群名
    API->>MSG: send_system "XXX 将群名修改为「YYY」"
    API->>DB: get_member_ids 查询群成员
    API->>WS: notify_group_info_update（推送给所有成员）
    API-->>C: 200 success
```
...
├── modules/
│   ├── flash-core/src/
│   │   └── error.rs                       # 新建：AppError 统一错误类型
│   ├── im-group/src/
│   │   ├── models.rs                      # 扩展：AddMembersRequest / TransferOwnerRequest / UpdateGroupRequest / UpdateAnnouncementRequest
│   │   ├── repository.rs                  # 扩展：add_members / remove_member / transfer / disband / update_announcement / update_group / get_member_ids
...
│   └── im-ws/src/
│       └── dispatcher.rs                  # 扩展：notify_group_info_update 方法
├── proto/
│   ├── ws.proto                           # 扩展：GROUP_INFO_UPDATE = 11
│   └── message.proto                      # 扩展：GroupInfoUpdate 消息
```
...
| 改名/改公告发系统消息 | routes 层发送系统消息（和 v0.0.1/v0.0.2 一致） | 保持一致的系统消息发送模式 |
| GROUP_INFO_UPDATE 独立帧类型 | 不污染 ConversationUpdate，新增 WsFrameType = 11 | ConversationUpdate 只负责消息预览和未读数，群信息变更语义不同 |
| 群成员查询群主置顶排序 | SQL 使用 `CASE WHEN cm.user_id = c.owner_id THEN 0 ELSE 1 END` | 群主始终排在成员列表第一位 |
| AppError 替代 map_err 反模式 | flash-core 新增 AppError 统一错误类型，实现 IntoResponse | 避免 `map_err(|_| StatusCode)` 丢失错误信息，500 时无法排查 |
...
flash-core → 新增 AppError（所有 crate 可用）
```
...
| 解散后发消息：被拦截 | 向已解散群发消息 |
| 群公告：发布/编辑成功 + 系统消息 + GROUP_INFO_UPDATE 推送 | PUT /groups/{id}/announcement |
| 修改群名：成功 + 系统消息 + GROUP_INFO_UPDATE 推送 | PUT /groups/{id} |
