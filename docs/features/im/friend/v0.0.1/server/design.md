...
- 新增 8 个 HTTP 接口：发送/接受/拒绝申请、查询收到/发送的申请、好友列表、删除好友、删除申请记录
- 新增用户搜索接口：GET /api/users/search（扩展 flash-user）
...
错误（JSON body `{"error": "具体原因"}`）：400（不能加自己/已是好友）、404（用户不存在）

> 实现变更：重复申请改为 upsert 覆盖旧申请，不再返回 400 AlreadyRequested。
...
└── src/
    ...
    └── api.rs                         # HTTP 路由：7 个好友接口

server/modules/flash-user/src/
    └── routes.rs                      # 修改：新增 GET /api/users/search
...
| 变更 | 原设计 | 实际实现 | 理由 |
|------|--------|---------|------|
| 重复申请策略 | 双向检查 pending 申请，重复返回 400 AlreadyRequested | 允许重复发送（upsert 覆盖旧申请） | 对齐微信行为，用户可修改留言重新发送；移除了 AlreadyRequested 双向检查 |
| 错误响应格式 | 仅返回 HTTP 状态码，无 body | 返回 `{"error": "具体原因"}` JSON body | 前端和测试脚本需要可读的错误信息 |
| 新增接口 | 7 个 HTTP 接口 | 8 个（新增 DELETE /api/friends/requests/:id） | 支持侧滑删除申请历史记录 |
| 接口总数 | 7 好友 + 1 搜索 = 8 | 8 好友 + 1 搜索 = 9 | — |
| 用户搜索匹配范围 | 仅昵称 ILIKE 模糊搜索 | 昵称 ILIKE + 手机号精确匹配（JOIN auth_credentials）+ 闪讯号精确匹配（account_id） | 对齐微信搜索体验，支持多种查找方式 |
