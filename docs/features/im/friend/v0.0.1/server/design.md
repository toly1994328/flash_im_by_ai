...
- 新增 7 个 HTTP 接口：发送/接受/拒绝申请、查询收到/发送的申请、好友列表、删除好友
...
错误：400（不能加自己/已发送过/已是好友）、404（用户不存在）
...
sequenceDiagram
    ...
    API->>DB: 校验：不能加自己、用户存在、非好友、无待处理申请
    API->>DB: INSERT friend_requests(status=0)
    API->>DB: 查询 A 的昵称/头像
