...
// 遍历 selectedIds，调 store.moveToTrash
// 内存过滤移除 + emit（静默，不调 loadMessages）
...
// store.moveToTrash + 内存过滤移除 + emit（静默）
// 调 _syncConversationPreview 更新会话预览
```
...
```dart
final VoidCallback? onConversationChanged;  // 构造函数参数

void _syncConversationPreview(LocalStore store, List<Message> messages)
// 取最后一条消息生成预览文本
// store.updateConversation 更新本地缓存
// onConversationChanged?.call() 通知会话列表刷新
```
...
void _handleMenuAction(MenuAction action, Message message) {
  switch (action) {
    ...
    case MenuAction.delete: _confirmDeleteMessage(context, cubit, message.id);
...
使用 `showTolyPopPicker`（tolyui_feedback_modal）弹出底部确认弹窗，红色"删除"按钮。
单条删除和批量删除共用同一模式。

### 14.5 ChatCubit 创建时注入 onConversationChanged `⬜`

home_page.dart 所有 ChatCubit 创建点传入回调：
