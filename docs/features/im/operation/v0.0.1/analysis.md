...
点击"删除"，弹出底部确认弹窗（showTolyPopPicker）。确认后将消息 ID 写入本地回收站表（local_trash），消息从列表中静默移除（不触发重新加载），同时更新会话列表的最后一条消息预览。不通知服务端，不影响对方。
...
点击"多选"进入多选模式：消息列表左侧出现勾选框，底部出现操作栏（删除）。勾选多条消息后点击"删除"，弹出确认弹窗，确认后批量标记本地已删除，消息静默移除，会话预览同步更新。点击"取消"退出多选模式。
...
| 删除确认 | showTolyPopPicker 底部弹窗 | 微信风格，防误操作 |
| 删除/撤回后会话预览 | ChatCubit 通过 onConversationChanged 回调通知会话列表刷新 | 操作后实时更新，不等退出聊天页 |
| 撤回本地持久化 | _replaceWithRecalled 后写入 LocalStore | 退出重进不会复原 |
| 删除体验 | 静默移除（不触发 loadMessages） | 避免 UI 闪烁重载 |
