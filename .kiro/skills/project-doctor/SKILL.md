---
name: project-doctor
description: 项目体检技能。在需要审查代码质量、排查潜在问题、评估架构健康度时激活。按望闻问切四步进行系统性检查。
metadata:
  model: manual
  last_modified: Tue, 13 May 2026 00:00:00 GMT

---
# 项目医生 — 望闻问切

## 角色

你是这个项目的主治医生。你的工作不是写新功能，而是检查项目的健康状况，找出潜藏的问题，给出诊断和处方。

## 体检流程

按"望闻问切"四步进行，每一步有明确的检查项和输出。

### 第一步：望（看结构）

运行统计脚本，产出模块文件数、行数、字符数、占比、大文件排行。

```bash
python scripts/checkup/client_stats.py    # 前端
python scripts/checkup/server_stats.py    # 后端
```

产出：
- 前端：`docs/project/checkup/{tag}/client/01_望_结构统计.md`
- 后端：`docs/project/checkup/{tag}/server/01_望_结构统计.md`

脚本自动完成数字统计后，AI 在 `03_问_代码审查.md` 中基于统计结果进行结构审查：
- 各模块占比是否均衡？有没有某个模块异常膨胀？
- shared 模块里的文件，是否真的被多个模块使用？
- 有没有废弃的文件/目录还留在项目里？
- 模块命名是否能让人一眼看出职责？
- 大文件排行里的文件，是否需要拆分？

### 第二步：闻（跑工具）

运行静态分析脚本，产出 error/warning/info 分类统计。

```bash
python scripts/checkup/client_analyze.py  # 前端（flutter analyze）
python scripts/checkup/server_analyze.py  # 后端（cargo clippy）
```

产出：
- 前端：`docs/project/checkup/{tag}/client/02_闻_静态分析.md`
- 后端：`docs/project/checkup/{tag}/server/02_闻_静态分析.md`

脚本自动完成分析后，AI 在 `03_问_代码审查.md` 中基于分析结果进行解读：
- warning 的共同特征是什么？（如"都是未使用的代码"说明有死代码堆积）
- info 里哪些值得关注？（如 `use_build_context_synchronously` 是潜在运行时风险）
- 和上次体检对比，问题是增多了还是减少了？

### 第三步：问（AI 审查）

AI 深入阅读关键模块的代码，从设计角度审查，结合望、闻阶段的结果，产出代码质量与问题汇报。

产出：`docs/project/checkup/{tag}/client/03_问_代码审查.md`（后端同理放 server 目录）

审查维度：
- **职责单一性**：这个类/模块是否只做一件事？有没有"顺手加进来"的逻辑？
- **依赖方向**：有没有下层依赖上层？有没有循环依赖？
- **接口合理性**：公开 API 是否最小必要？有没有暴露内部实现细节？
- **一致性**：相似的问题在不同模块里的处理方式是否一致？
- **命名**：变量名、方法名、文件名是否准确表达了意图？
- **错误处理**：异常路径是否被覆盖？有没有吞掉错误的 catch？
- **生命周期**：Stream 订阅有没有对应的取消？Controller 有没有 dispose？

审查范围（按优先级）：
1. 望和闻中发现的问题文件（大文件、warning 集中的文件）
2. 跨模块的公共接口（barrel file / 公开 API）
3. 状态管理层（Cubit / State）
4. 数据层（Repository / LocalStore）

报告内容：按模块分组，每个问题标注严重程度 + 具体位置 + 问题描述。

### 第四步：切（深入诊断）

针对前三步发现的疑似问题，深入确认。

操作：
- 对"望"发现的结构问题：打开具体文件，确认是否真的有问题
- 对"闻"报出的 warning：判断是误报还是真问题
- 对"问"发现的设计问题：评估影响范围和修复代价

输出：确诊问题清单，分三级：
- 🔴 严重：阻塞后续开发或有运行时风险，建议立即修复
- 🟡 一般：不影响功能但影响可维护性，建议近期修复
- 🟢 轻微：代码风格或命名问题，可以顺手修或暂时不管

## 体检报告格式

每端各产出一份独立的体检目录，按版本号命名。

产出位置：
- 前端：`docs/project/checkup/{tag}/client/`
- 后端：`docs/project/checkup/{tag}/server/`

目录结构：
```
docs/project/checkup/{tag}/
├── client/
│   ├── 01_望_结构统计.md    ← 脚本自动生成
│   ├── 02_闻_静态分析.md    ← 脚本自动生成
│   ├── 03_问_代码审查.md    ← AI 审查后记录
│   └── 04_切_确诊处方.md    ← 汇总确诊 + 处方
└── server/
    ├── 01_望_结构统计.md
    ├── 02_闻_静态分析.md
    ├── 03_问_代码审查.md
    └── 04_切_确诊处方.md
```

03 和 04 由 AI 基于前两步的产出进行深入分析，写入对应文件。格式如下：

```markdown
# {端} 代码审查（问）

日期：YYYY-MM-DD　版本：{tag}

## 审查范围

...

## 发现

### 模块 A

...

### 模块 B

...
```

确诊处方格式：

```markdown
# {端} 确诊与处方（切）

日期：YYYY-MM-DD　版本：{tag}

## 确诊清单

| # | 级别 | 问题 | 位置 | 处方 |
|---|------|------|------|------|
| 1 | 🔴 | ... | ... | ... |
| 2 | 🟡 | ... | ... | ... |
| 3 | 🟢 | ... | ... | ... |

## 医嘱

- 立即处理的事项
- 近期关注的事项
- 长期改善建议
```

## 使用方式

用户说"给项目做个体检"、"检查一下代码质量"、"项目有什么问题"时激活。

体检可以是全面的（四步都走），也可以是局部的（只看某个模块、只跑静态分析）。根据用户需求灵活调整范围。

## 第五步：治（修复）

用户说"开始处理问题"、"修复体检发现的问题"时进入此阶段。

流程：
1. 基于当前版本创建新分支（如 `v0.20.0_fix`），不打 tag
2. 按确诊处方的优先级逐个修复
3. 每次修复后跑 `flutter analyze` / `cargo clippy` 验证
4. 全部完成后产出 `05_治_修复记录.md`，放在**体检报告所在目录**（如 `docs/project/checkup/{tag}/client/05_治_修复记录.md`）
5. 分支合并回主线即可，不需要归档或打新 tag

修复记录格式：

```markdown
# {端} 修复记录（治）

日期：YYYY-MM-DD　版本：{from_tag} → {to_tag}

## 修复成果

| 指标 | 修复前 | 修复后 | 变化 |
|------|--------|--------|------|
| 🔴 error | x | 0 | -x |
| 🟡 warning | x | 0 | -x |
| 🟢 info | x | 0 | -x |

## 修复明细

（按优先级分组，列出每个问题的修复方式）

## 收获与沉淀

（新建的规范、工具、包结构调整等）
```

