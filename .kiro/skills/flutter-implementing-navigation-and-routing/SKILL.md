...
- [Implementing Imperative Navigation](#implementing-imperative-navigation)
  - [Avoiding Pop-then-Push Animation Conflicts](#avoiding-pop-then-push-animation-conflicts) [toly+]
...
### Avoiding Pop-then-Push Animation Conflicts [toly+]

When a page collects user input and then navigates to a result page, avoid the pattern `await Navigator.push() → pop returns result → push next page`. The pop animation and the subsequent push animation overlap, causing both pages to be visible simultaneously.

**Preferred pattern: callback + pushReplacement.** Instead of popping with a result, pass an `onComplete` callback into the form page. In the callback, use `Navigator.pushReplacement()` to swap the form page for the result page in a single transition. The user sees the form page slide away and the result page slide in, with no intermediate flash of the underlying page.

- Use `onComplete` callback instead of `Navigator.pop(context, result)` when the caller needs to navigate after receiving the result.
- Use `Navigator.pushReplacement()` inside the callback to replace the current route with the next one.
- This keeps the navigation stack clean: FormPage is replaced by ResultPage, and pressing back returns to the original page.
...
### Example: Callback + pushReplacement (Avoiding Animation Conflicts) [toly+]

```dart
// ❌ Bad: pop + push causes overlapping animations
final result = await Navigator.push(context, MaterialPageRoute(
  builder: (_) => FormPage(),
));
if (result != null) {
  Navigator.push(context, MaterialPageRoute(
    builder: (_) => ResultPage(data: result),
  ));
}

// ✅ Good: callback + pushReplacement for seamless transition
Navigator.push(context, MaterialPageRoute(
  builder: (_) => FormPage(
    onComplete: (result) {
